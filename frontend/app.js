...
function cacheElements() {
  ...
  els.routeBadge = document.getElementById("route-badge");
...
async function checkHealth({ showChecking = true } = {}) {
  ...
  try {
    const response = await fetch(`${API_BASE}/health`, { signal: timeoutSignal(HEALTH_TIMEOUT_MS) });
    const data = await response.json();
    ...
    if (requestId !== state.healthRequestId) return;
    els.statusLine.textContent = "waking the server (~40s)";
    const retryDelay = Math.max(0, HEALTH_RETRY_MS - (Date.now() - startedAt));
    state.healthRetryId = window.setTimeout(() => {
      state.healthRetryId = null;
      checkHealth({ showChecking: false });
    }, retryDelay);
  }
}

function clearHealthRetry() {
  if (state.healthRetryId !== null) {
    window.clearTimeout(state.healthRetryId);
    state.healthRetryId = null;
  }
}
...
async function runQuery(query, context) {
  ...
  // A completed query proves the service is awake. Refresh the header so a
  // cold-start health failure cannot leave stale warning copy behind.
  checkHealth();
...
// v2 routing badge tells the reader HOW the answer was produced, in plain
// language. This is the trust line: a value from the facts database was
// copied by deterministic code rather than generated by a language model;
// a synthesized answer had its citations checked against retrieved evidence.
function renderRoute(meta) {
  if (!els.routeBadge) return;
  const route = meta.route;
  if (!route) {
    els.routeBadge.hidden = true;
    return;
  }
  ...
  if (route === "factual_lookup") {
    cls += " route-factual";
    text = "deterministic · value copied from the verified facts file · source quote and page below";
    if (meta.fact_status === "draft") {
      text += " · draft fact, human verification pending";
    }
  } else if (route === "clarification") {
    cls += " route-clarify";
    text = "needs detail · more than one stored value fits this question · it asks before answering";
  } else {
    cls += " route-synth";
    const v = meta.validation;
    if (v && v.status === "passed") {
      text = "AI-synthesized · every citation checked against the retrieved sources · numbers verified";
    } else if (v && (v.status === "flagged" || v.status === "refused")) {
      const problems = [];
      if ((v.invalid_citations || []).length) problems.push(`${v.invalid_citations.length} citation(s) not in retrieved sources`);
      if ((v.ungrounded_numbers || []).length) problems.push(`number(s) not found in sources: ${v.ungrounded_numbers.slice(0, 3).join(", ")}`);
      if (!problems.length) problems.push("low source support");
      text = `AI-synthesized · validation flagged: ${problems.join(" · ")}`;
      cls += " route-flagged";
    } else if (v && v.status === "not_applicable") {
      text = "AI-synthesized · declined, the shelf doesn't cover this";
    } else {
      text = "AI-synthesized · grounded in the retrieved sources";
    }
  }

  els.routeBadge.className = cls;
...
function startWarmMeter(host, options) {
  ...
  function show() {
    ...
    update(0, "seconds waiting for response", false);
    options.onShow?.();
    options.onLog?.("> request still running · the free service may be waking or generating");
    options.onLog?.("> elapsed time shown · this is not server progress");
  ...
  function tick() {
    ...
    if (elapsed <= 60) {
      update(elapsed, "seconds waiting for response", false);
    } else {
      update(elapsed, "seconds waiting · longer than usual", true);
  ...
  function update(value, label, overrun) {
    ...
    const markerSeconds = Math.min(60, value);
  ...
  return {
    ready() {
      ...
      options.onLog?.(`> response received in ${measured} s`);
  ...
  return `
    <div class="warm-meter${compact ? " compact" : ""}">
      ...
      <div class="warm-readout">
        <strong data-warm-number>0</strong>
        <span data-warm-label>seconds waiting for response</span>
      </div>
      <p>> elapsed request time · the delay may be startup, retrieval, or generation</p>
      <p>${COLD_START_LINE}</p>
    </div>
  `;
}
